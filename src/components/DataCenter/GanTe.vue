        ...
        <div class="MessageWrap">
            <ul class="Message">
                <li v-for="item in PlanLine" :key="item.uid">
                    <div class="plan" :style="{'left':item.left+'px','width':item.width+'px'}">
                        <div v-if="item.finishWidth!==0" class='finish' :style="{'width':item.finishWidth+'%'}">{{item.finishedQty}}</div>
                        <div v-if="item.unFinishWidth!==0" class="unFinish" :style="{'width':item.unFinishWidth+'%'}">{{item.plannedQty - item.finishedQty }}</div>
                    </div>
                </li>
...
<style scoped>
    ...
    #Wrap .OpWrap::-webkit-scrollbar{
        width:2px;
        height:5px;
        /**/
    }
    #Wrap .OpWrap::-webkit-scrollbar-track{
        background: rgb(239, 239, 239);
        border-radius:2px;
    }
    #Wrap .OpWrap::-webkit-scrollbar-thumb{
        background: #bfbfbf;
        border-radius:10px;
    }
    #Wrap .OpWrap::-webkit-scrollbar-thumb:hover{
        background: #333;
    }
    #Wrap .OpWrap::-webkit-scrollbar-corner{
        background: #179a16;
    }
    #Wrap .OpName{
        padding: 0;
    ...
    #Wrap .TimerWrap::-webkit-scrollbar{
        width:2px;
        height:5px;
    }
    ...
    #Wrap .Timer li:not(:first-child) p::before{
        display: block;
        position: absolute;
        content: "";
        width: 1px;
